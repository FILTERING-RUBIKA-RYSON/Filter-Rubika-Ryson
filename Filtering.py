# MMD RYSON
# py3
# Khsteh Az Hameh
print("")
print("")
print("""
 ▄▄▄  ▄▄▄
 ███  ███
 ████████
 ██ ██ ██
 ██ ▀▀ ██
 ██    ██
 ▀▀    ▀▀""")
print(" ")
print("""
 ▄▄▄  ▄▄▄
 ███  ███
 ████████
 ██ ██ ██
 ██ ▀▀ ██
 ██    ██
 ▀▀    ▀▀""")
print(" ")
print("""
 ▄▄▄▄▄
 ██▀▀▀██
 ██    ██
 ██    ██
 ██    ██
 ██▄▄▄██
 ▀▀▀▀▀""")
print (" ")
print("updated Load ...")                 
import os
#__
os.system("pip install colored")
os.system("pip install colorama")
os.system("pip install datetime")
#importing
import webbrowser
import time                                                    
import sys                                                      
import colored                                                  
import colorama                                                 
import datetime
from colored import fg, bg, attr
from colorama import Fore, Back, Style                          
# colors
red='\033[31m'
green='\033[32m'
blue='\033[36m'
pink='\033[35m'
rang='\033[34m'


os.system("clear")

print(f"{green} ")

print (Fore.WHITE + "")
print (" Start Code Filter Rubika ~CrEaToR_rYsOn")
print (" ")
#date
x = str(datetime.datetime.now())

print("Beginning " + (x))
print (" ")
print(f"{blue} ")
print ("         GOD MAJAZI ")
print (" ")
print ("   They are the only creators of (ryson) mammad")
print (" ")
print ("""The best filtering rule in virtual There are ryson! rules.""")
print (" ")
print (Fore.YELLOW + "")
print (""" Supermarket code reporter in virtual by mmd """)
print (Fore.GREEN + "")
x = """ Loading Code..

R4S0N---STARTED-12
                  .xUHWH!! !!?RYSONN:.
                .X*#M@$!!  !X!M$$$$$$WWx:.
               :!!!!!!?H! :!$!$$$$$$$$$$8X:
              !!~  ~:~!! :~!$!#$$$$$$$$$$8X:
             :!~::!H!<   ~.U$X!?R$$$$$$$$MM!
             ~!~!!!!~~ .:XW$$$U!!?$$$$$$RMM!
               !:~~~ .:!M"T#$$$$WX??#MRMMDM!
              ~?WuxiW*   "#$$$$8!!!!??!!!     
             :X- M$$$$       "T#$T~!8$WUXU~
             :%  ~#$$$m:        ~!~ ?$$$$$$
          :!.-   ~T$$$$8xx.  .xWW- ~""##*"
.....   -~~:< !    ~?T#$$@@W@*?$$      /
W$@@M!!! .!~~ !!     .:XUW$W!~ "~:    :
#"~~.:x%!!  !H:   !WM$$$$Ti.: .!WUn+!
:::~:!!:X~ .: ?H.!u "$$$B$$$!W:U!T$$M~
.~~   :X@!.-~   ?@WTWo("*$$$W$TH$!          
Wi.~!X$?!-~    : ?$$$B$Wu("**$RM!          
$R@i.~~ !     :   ~$$$$$B$$en:`            
?MXT@Wx.~    :     ~"##*$$$$M~                


version 3.0.1                                    
"""
for c in x:
    sys.stdout.write(c)
    sys.stdout.flush()
    time.sleep(0.01)

print(f"{red} ")
d = """

I Love all

 ▄▄▄▄▄▄   ▄▄▄    ▄▄▄   ▄▄▄▄      ▄▄▄▄    ▄▄▄   ▄▄
 ██▀▀▀▀██  ██▄  ▄██  ▄█▀▀▀▀█    ██▀▀██   ███   ██
 ██    ██   ██▄▄██   ██▄       ██    ██  ██▀█  ██
 ███████     ▀██▀     ▀████▄   ██    ██  ██ ██ ██
 ██  ▀██▄     ██          ▀██  ██    ██  ██  █▄██
 ██    ██     ██     █▄▄▄▄▄█▀   ██▄▄██   ██   ███
 ▀▀    ▀▀▀    ▀▀      ▀▀▀▀▀      ▀▀▀▀    ▀▀   ▀▀▀

                            
            god filtering rubika mmd R-yson

                      /MMD ~> ryson /-

                    CODE FILTERI RUBIKA

      hazrat  i am (peter) _ afshar  ≈ (ryson)

       WEBLOG GHAVANIN ~> conquest-law.blogfa.com
          
     


starting |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||| _ _ _ _
"""
for d in d:
        sys.stdout.write(d)
        sys.stdout.flush()
        time.sleep(0.03)


# Chaker * _ *  Hal Kardi?

time.sleep(2)
print(f"{pink} ")
print ("Play - - - ")
print (Fore.BLUE + "")
print(f"{blue} ")
time.sleep(0.6)
print ("               !")
time.sleep(0.2)
print ("               !!")
time.sleep(1.0)
print ("              !!!!")
time.sleep(0.6)
print ("             I!!!!I")
time.sleep(0.2)
print ("            M!!!!!!M")
time.sleep(0.6)
print ("           M!!!!!!!!!M")
time.sleep(0.4)
print ("          D!!!!!!!!!!!D")
time.sleep(0.4)
print ("         R!!!!!!!!!!!!!!R")
time.sleep(0.3)
print ("        Y!!!!!!!!!!!!!!!!Y")
time.sleep(0.2)
print ("       S!!!!!!!!!!!!!!!!!!S")
time.sleep(0.1)
print ("      O!!!!!!!!!!!!!!!!!!!!O")
time.sleep(0.5)
print ("     N!!!!!!!!!!!!!!!!!!!!!!N")
time.sleep(0.4)
print ("    A!!!!!!!!!!!!!!!!!!!!!!!!A")
time.sleep(0.3)
print ("   M!!!!!!!!!!!!!!!!!!!!!!!!!!M")
time.sleep(0.2)
print ("   M!!!!!!!!!!!!!!!!!!!!!!!!!!M")
time.sleep(0.1)
print ("    A!!!!!!!!!!!!!!!!!!!!!!!!!A")
time.sleep(0.1)
print ("      M!!!!!!!!!!!!!!!!!!!!!!M")
time.sleep(0.3)
print ("       A!!!!!!!!!!!!!!!!!!!!A")
time.sleep(0.2)
print ("        D!!!!!!!!!!!!!!!!!!D")
time.sleep(0.2)
print ("         R!!!!!!!!!!!!!!!!R")
time.sleep(0.1)
print ("          Y!!!!!!!!!!!!!!Y")
time.sleep(0.3)
print ("           S!!!!!!!!!!!!S")
time.sleep(0.4)
print ("            O!!!!!!!!!!O")
time.sleep(0.3)
print ("             N!!!!!!!!N")
time.sleep(0.2)
print ("              A!!!!!!A")
time.sleep(0.1)
print ("               M!!!!M")
time.sleep(1.0)
print ("                B!!B")
time.sleep(0.3)
print ("                 YY")
time.sleep(0.3)
print ("                 E")
time.sleep(0.2)
print ("")
print (" ")
print ("")
# input
up = input(""" 

- [Chanel    (1) ] -

- [Grup      (2) ] -
                 
- [Account   (3) ] -
                  
- [Super     (0) ] -

- [Exit      (4) ] -

- [Code Pd   (5) ] -

- [Code Rp   (6) ] -

- [Reporter  (7) ] -

- [Sms Bomb  (8) ] -

- [Hack_Rubk (9) ] -

- [up ≠chanl (10)] -

        [Which do you want]->>> """)
print("")
print (" ")
print(f"{pink} ")
time.sleep(2.0)
s = """

 
               >       off rubika.ir/Spansor             <
               >     online shad.ir/creator_ryson        <
               >    Fil rubika.ir/Conquest_Rayson        <
               >   telegram on  t.me/Creator_Ryson       <
               > Weblog ghavanin: CONQUEST-LAW.blogfa.com<
               >           creatar.blogfa.com            <

                                       
 
                               
 
                       🇴‌🇵‌🇪‌🇳‌  🇷‌🇪‌🇵‌🇴‌🇷‌🇹‌‌‌‌‌

                 |--------------------------------|
                 |>Web= creator-rayson.blogfa.com<|
                 |>|>|>|>|>|>|>|>|>|>|>|>|>|>|>|>||
                 |_____[telegram.me/spansor]______|



                       ###############
                      #################            #
                   ######################         #
                  #########################      #
                ############################
               ##############################
               ###############################
              ###############################
              ##############################
                              #    ########   #
                 ##        ###        ####   ##
                                      ###   ###
                                    ####   ###
               ####          ##########   ####
               #######################   ####
                 ####################   ####
                  ##################  ####
                    ############      ##
                       ########        ###
                      #########        #####
                    ############      ######
                   ########      #########
                     #####       ########
                       ###       #########
                      ######    ############
                     #######################
                     #   #   ###  #   #   ##
                     ########################
                      ##     ##   ##     ##

"""
for r in s:
    sys.stdout.write(r)
    sys.stdout.flush()
    time.sleep(0.002)

print (" ")
print ("")
print(f"{red} ")

if up == "10":
    webbrowser.open("https://rubika.ir/caetorr")


if up == "9":
    os.system("cd && rm -rf Hack-Rubika && git clone https://github.com/Filtering-Rubika-Ryson/Hack-Rubika && cd Hack-Rubika && chmod 777 * && python rubika.py")
if up == "8":
    os.system("python sms.py")
if up == "7":
    os.system("python rep.py")
if up == "6":
    os.system("python report.py")
if up == "5":
    os.system("python pass.py")
if up == "4":
    os.system("exit")
    
if up == "0":
         print ('!Code filter (SUPER) ≈>>>' + '%s%s (8.6.0.3/e///f.h/0.0.1.3.8.9.0.1.3.9.0.1.3.0.7.0.6.0.9.2!/) %s' % (fg('red'), bg('black'), attr('reset')))


if up == "1":
        print ('!Code filter (CHANNEL) ≈>>>' + '%s%s (3.0.6.0/y//f/h/4.1.3.0.8.0.1.7.5.0.1.2.7.0.1.9.3.7.0.3.2*/) %s' % (fg('red'), bg('green'), attr('reset')))

if up != "1":
        print("""    
                 COD 
 """)

if up == "2":
        print ('!Code Filter (GROUP) ≠>>>' + '%s%s (4.0.7.5//f/h////6.0.9.1.5.1.7.2.0.8.6.1.7.0.5.4.7.0.0.5) %s' % (fg('red'), bg('white'), attr('reset')))


if up != "2":
        print("""
                        cσ∂s мү яερσят
  """)


if up == "3":
        print ('!Code Filter (ACCOUNT) ≠>>>' + '%s%s (2.1.0.4/f//a/y/9.0.8.1.3.0.1.7.6.0.7.4.8.9.1.0.3.8.6.0.2.4.9.0/) %s' % (fg('red'), bg('white'), attr('reset')))


if up != "3":
        print("""             
                               anti filter , no filtering: (127.0.0.1:8080{r430n}) !For Channel Biography!
        
          Khalegh Ryson""")

time.sleep(0.6)
print(f"{green}" )
print ("        S'cary")
print(f"{blue}" )
print(" ")
print("     Web Cod=                      code-filtring.blogfa.com ")
print("")
print("     Bot Cod= t.me/code_filtering_bot -vs- t.me/spansor_bot ")
print("")
print(" server: ///g///h ")
print("")
print("       channel telegram≈>> t.me/uupdatte ")
print("""     channel rubika≈>> rubika.ir/caetorr""")
print (Fore.WHITE + " ")
time.sleep(2.0)
print ("      offline")
time.sleep(3.0)
print (f"{red} ")
time.sleep(1)
print ('Cod Ro Dorost Bezan 70% Filter Mikone  ' + '%s%s TanK You My Friend  %s'  % (fg('red'), bg('white'), attr('reset')))
print (f"{blue} ")
time.sleep(1)
print (""" {code ro ( 5 ) bar bezanin to Sayer!!}
         Tanks""")
print(" ")
print(" ")
print(" ")
print ("          THE END!")
print("")
print(""" (Update Kardan Script) 👇:

cd

rm -rf Filter-Rubika-Ryson 


git clone https://github.com/Filtering-Rubika-Ryson/Filter-Rubika-Ryson


chmod 777 *


cd Filter-Rubika-Ryson


python3 Filtering.py

""")
print("")
print("")
time.sleep(0.3)
print(f"{red}" )
time.sleep(0.5)
print("""                        ghalb mmd>  </💔> """)
print (Fore.GREEN + """ 
           #666 Super Script 
                 Finished Code..
                  24 Hours to update version """)
input()
os.system("python Filtering.py")
     
